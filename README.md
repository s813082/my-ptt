...
每次 Workflow 觸發時，會依序執行以下步驟：

```
┌─────────────────────────────────────────────────────────┐
│           GitHub Actions 觸發（每分鐘 1 次）              │
└────────────────────────┬────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│  步驟 1：從 GitHub Cache 還原 seen_posts.json             │
│  • Key: drama-ticket-seen-posts（固定名稱，確保連續性）    │
│  • 若 Cache 不存在（首次執行）→ 從空白開始                 │
└────────────────────────┬────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│  步驟 2：執行 scraper.py                                  │
│                                                         │
│  for 每篇文章 in PTT Drama-Ticket 最新 10 頁:             │
│    ├── 文章已在 seen_posts.json 中？ → 跳過               │
│    ├── 標題不含「售票」？         → 跳過               │
│    ├── 內文不含「孫燕姿」？       → 跳過               │
│    └── 全部通過 ──────────────────────────────────►     │
│                                                         │
...
│                         │                               │
│                         ▼                               │
│              將文章 ID 加入 seen（不再重複通知）            │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│  步驟 3：將更新後的 seen_posts.json 存回 GitHub Cache     │
│  • 同樣的 key → 覆蓋舊版，確保下次啟動能繼承記憶           │
│  • 最多保留 500 筆，自動淘汰最舊記錄                       │
└─────────────────────────────────────────────────────────┘
```

> **📌 關於 Cache 存活時間**：GitHub Actions Cache 在 **7 天內沒有存取**會自動清除。本專案每分鐘執行一次，完全不會觸發此限制。若手動停用排程超過 7 天後重啟，首次執行會遺失舊紀錄、可能重複通知歷史文章，屬正常現象。
